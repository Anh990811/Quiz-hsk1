...
function startRPRecording() {
  ...
  if (mode === 'none') {
    showToast('⚠️ Thiết bị không hỗ trợ ghi âm.', 4000);
    return;
  }

  DialogueState.isRecording = true;

  var btn = document.getElementById('rp-record-btn');
  var lbl = document.getElementById('rp-record-label');
  var wv  = document.getElementById('rp-waveform');
  ...
  if (mode === 'speech-api') {
    try { DialogueState.recognition.start(); } catch(e) { /* đang chạy rồi */ }
  } else {
    // MediaRecorder fallback
    var result = document.getElementById('rp-result');
    if (result) result.innerHTML = '<span class="rp-interim">🎙️ Đang ghi âm...</span>';
    RecorderUtil.startMediaRecord(function(audioURL) {
      DialogueState.isRecording = false;
      _stopRPRecordUI();
      if (audioURL) showRPMediaResult(audioURL);
    });
  }
}

function _stopRPRecordUI() {
  var btn = document.getElementById('rp-record-btn');
  var lbl = document.getElementById('rp-record-label');
  var wv  = document.getElementById('rp-waveform');
...
function stopRPRecording() {
  ...
  if (RecorderUtil.mode === 'speech-api') {
    if (DialogueState.recognition) { try { DialogueState.recognition.stop(); } catch(e) {} }
  } else {
    RecorderUtil.stopMediaRecord();
  }
}

/* ---- MediaRecorder self-assessment UI (role-play) ---- */
function showRPMediaResult(audioURL) {
  var dlg  = DialogueState.dialogue;
  var line = dlg && dlg.lines[DialogueState.currentLineIndex];
  var result = document.getElementById('rp-result');
  if (!result) return;

  var chinese = line ? line.chinese : '';
  var pinyin  = line ? line.pinyin  : '';
  ...
  result.innerHTML =
    ...
    '<div class="mr-btns">' +
      '<button class="mr-btn mr-great" onclick="submitRPScore(95)">🎉 Xuất sắc</button>' +
      '<button class="mr-btn mr-ok"    onclick="submitRPScore(70)">👍 Khá tốt</button>' +
      '<button class="mr-btn mr-retry" onclick="submitRPScore(30)">💪 Thử lại</button>' +
    '</div></div>';
}

function submitRPScore(score) {
  var dlg  = DialogueState.dialogue;
  var line = dlg && dlg.lines[DialogueState.currentLineIndex];
  var result = document.getElementById('rp-result');
  ...
  // Add to chat history
  if (line) addRPChatBubble(line, true, pct);

  DialogueState.lineResults.push(pct);
  updateRPScore();

  if (result) {
    result.innerHTML = '<span style="font-weight:600">' + emoji + ' Bạn tự chấm: ' + pct + '%</span>';
  }
...
function setupDialogueRecognition() {
  ...
  if (mode !== 'speech-api') {
    // Không có Speech API – dùng MediaRecorder
    DialogueState.recognition = null;
    return;
  }
  ...
  var r  = new SR();
  r.lang = 'zh-CN';
  r.continuous = true;          // Ghi âm liên tục
  ...
  // Tự restart nếu trình duyệt ngắt mà người dùng chưa bấm Dừng
  r.onend = function() {
    if (DialogueState.isRecording) {
      try { DialogueState.recognition.start(); } catch(e) { stopRPRecording(); }
    }
  };
  ...
  r.onerror = function(err) {
    ...
    var msg = errMap.hasOwnProperty(err.error) ? errMap[err.error] : ('⚠️ Lỗi ghi âm: ' + err.error);
