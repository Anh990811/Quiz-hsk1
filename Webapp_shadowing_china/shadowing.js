...
function showLessonView(topicIndex) {
  ShadowState.currentTopicIndex = topicIndex;
  ...
  const topic = ShadowState.topics[topicIndex];
  document.getElementById('lesson-icon').textContent    = topic.icon;
  document.getElementById('lesson-title').textContent   = topic.topic;
...
function renderTopicGrid() {
  ...
  ShadowState.topics.forEach(function(topic, index) {
    ...
    card.innerHTML =
      ...
      '<div class="topic-count">' + done + '/' + total + ' câu đã luyện</div>' +
...
function loadSentence(index) {
  const topic = ShadowState.topics[ShadowState.currentTopicIndex];
  if (!topic) return;
...
function nextSentence() {
  ...
  else showToast('🎉 Đã hoàn thành chủ đề này!');
}
...
function markSentenceDone(sIndex, topicId, topicIndex) {
  ...
  if (card) {
    ...
    if (cnt) cnt.textContent = done + '/' + total + ' câu đã luyện';
  }
}
